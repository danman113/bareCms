...
class CmsInstance(threading.Thread):
	def __init__(self):
		...
		self.ps = None
	def run(self):
		self.ps = subprocess.Popen(["node", "../index.js", "--db.filename=./database.sql", "--router.staticURL=./staticTest"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	def kill(self):
		self.ps.kill()
	def get_error(self):
		return self.ps.stdout.readline()

def register_and_login(username="testerman",password="wordpass123"):
	data = {
		"username": username,
		"password": password,
		"password2": password,
...
def test_file_upload():
	...
	files = {'uploads': open( test_file ,'r' )}
	text = open( test_file ,'r' ).read()
	file_upload1 = s.post('http://localhost:8080/files/upload', files=files, data={ 'folder': '' })
	assert file_upload1.status_code == 201
	assert file_upload1.json()['status'] == 1

	file_download1 = s.get('http://localhost:8080/static/' + test_file, stream=True)
	assert file_download1.status_code == 200
	assert file_download1.raw.read() == text

	folder_upload = s.put('http://localhost:8080/files/upload', data={ 'folderName': 'newFolder', 'folder': '' })
	assert file_upload1.status_code == 201
	assert file_upload1.json()['status'] == 1

	files = {'uploads': open( test_file ,'r' )}
	file_upload2 = s.post('http://localhost:8080/files/upload', files=files, data={ 'folder': 'newFolder' })
	assert file_upload2.status_code == 201
	assert file_upload2.json()['status'] == 1

	file_download2 = s.get('http://localhost:8080/static/newFolder/' + test_file, stream=True)
	assert file_download2.status_code == 200
	assert file_download2.raw.read() == text
	time.sleep( 2 )
